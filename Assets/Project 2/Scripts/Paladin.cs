...
public class Paladin : Unit {
	...
	{
		if (gameObject.name.Equals("Paladin-Red(Clone)")) { // red
			animator.SetLayerWeight (2, 1f);
			animator.SetLayerWeight (1, 0f);
			animator.SetLayerWeight (0, 0f);
		} else if (gameObject.name.Equals("Paladin-Green(Clone)")) { // green
			animator.SetLayerWeight (1, 1f);
			animator.SetLayerWeight (0, 0f);
			animator.SetLayerWeight (2, 0f);
		} else if (gameObject.name.Equals("Paladin-Blue(Clone)")){// blue
	...
	protected override IEnumerator WaitForAbility ()
	{
		...
		while (isAttacking) {
			if (Input.GetMouseButtonDown (0))
			{
				...
				else if (hit.collider != null && abilityRange >= distance && hit.collider.transform.tag.Equals("Player1"))
				{
					...
					photonView.RPC("swapPos", PhotonTargets.AllBufferedViaServer, hit.collider.transform.position);
					Debug.Log("friend selected" );
					HighlightTargets (validTargets, Color.white);
					HighlightUnit(Color.yellow);
					photonView.RPC("setWalkkAnimation", PhotonTargets.AllBufferedViaServer);
					hasAttacked = true;
					isAttacking = false;
				}
				else{
	...
	[PunRPC] void swapPos(Vector3 targetPos)
	{
		...
		if (hit.collider != null) {
			...
			Debug.Log("swapping positions with " + gameObject.name);
			var temp = targetUnit.transform.position;
